...
class ProjectController(app_manager.RyuApp):
    ...
    def get_equal_cost_paths(self, src, dst):
        """
        Find paths with equal cost between src and dst for ECMP
        """
        paths = self.get_paths(src, dst)
        if not paths:
            return []

        path_costs = {tuple(path): self.get_path_cost(path) for path in paths}
        min_cost = min(path_costs.values())

        equal_cost_paths = [list(path) for path, cost in path_costs.items() if cost == min_cost]
        return equal_cost_paths[:MAX_PATHS]
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        paths = self.get_equal_cost_paths(src, dst)
